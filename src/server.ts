...
mongoose
    ...
    .then(async () => {
        ...
        for (const bot of runningBots) {
            try {
                await startBotJob(
                    (bot._id as mongoose.Types.ObjectId).toString()
                );
                scheduledCount++;
            } catch (err) {
                console.error(
                    `Failed to schedule bot ${(bot._id as mongoose.Types.ObjectId).toString()}:`,
                    err
                );
            }
        }
        console.log(
            `Scheduled ${scheduledCount} running bot(s) on startup.`
        );
